...
def load_deca_params(deca_dir, cfg):
    ...
    # face params 
    params_key = ['shape', 'pose', 'exp', 'cam', 'light', 'faceemb', 'tform', 'albedo', 'detail', 'shadow', 'skincolor']
    ...
    *,
    ...
    for in_image_type in condition_image + input_image:
        if in_image_type is None: continue
            ...
            if 'deca' in in_image_type:
            ...
            elif 'albedo' in in_image_type:
                in_image[in_image_type] = _list_image_files_recursively(f"{cfg.dataset.albedo_rendered_dir}/{in_image_type}/{set_}/")
                ...
                in_image[in_image_type] = _list_image_files_recursively(f"{cfg.dataset.shadow_mask_dir}/")
...
class DECADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i, k in enumerate(self.condition_image):
            ...
            elif 'deca_masked_face_images_woclip' in k:
            ...
            elif 'albedo_masked_face_images_woclip' in k:
                shadow_mask_weight = np.array(self.load_image('./preprocess_scripts/ffhq/albedo_images/albedo_masked_face_images_wclip/valid/'+query_img_name))
                shadow_mask_weight = cv2.resize(shadow_mask_weight, (self.resolution, self.resolution), cv2.INTER_AREA)
                shadow_mask_weight = np.transpose(shadow_mask_weight, [2, 0, 1])
                shadow_mask_weight = shadow_mask_weight / 255.0
                each_cond_img = shadow_mask_weight 
                cond_img[k] = each_cond_img
                out_dict[f'{k}_img'] = each_cond_img 
            elif 'laplacian' in k:
