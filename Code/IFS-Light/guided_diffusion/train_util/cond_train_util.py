...
from ..models.nn import update_ema, conv_nd, avg_pool_nd
...
class TrainLoop(LightningModule):
    ...
    def forward_cond_network(self, cond, model_dict=None):
        ...
        if self.cfg.img_cond_model.apply:
            ...
            if self.cfg.img_cond_model.override_cond != "":
                ...
                #加了skincolor
                if self.cfg.img_cond_model.override_cond in ['shape', 'pose', 'exp', 'cam', 'light', 'faceemb', 'img_latent', 'skincolor']:
                    tmp = []
    ...
    def forward_backward(self, batch, cond):
        ...
        albedo = cond['albedo_masked_face_images_woclip_img']
        import torchvision
        torchvision.utils.save_image(albedo, 'albedo.png')

        shadow = cond['deca_masked_face_images_woclip_img']
        import torchvision
        torchvision.utils.save_image(shadow, 'shadow.png')

        #把这个注释掉了
        bg_tmp = cond['faceseg_nohead_img'] 
        mask_bg = cond['faceseg_nohead_mask']
        mask = (mask_bg == 1)
        #从0.5改到了0.25
        bg_tmp[mask] = 0.5
        cond['faceseg_nohead_img']  = bg_tmp

        # import torchvision
        # torchvision.utils.save_image(bg_tmp, 'bg.png')

        # exit()

        # print(th.mean(cond['light']))
        # import torchvision
        # tmp_img = bg_tmp
        # tmp_img = (tmp_img + 1) * 127.5 / 255.0
        # torchvision.utils.save_image(tmp_img, 'bg.png')
        # cond['faceseg_nohead_img'] += 0.1*th.mean(cond['light'][0:3])
        # import torchvision
        # tmp_img = cond['faceseg_nohead_img']
        # tmp_img = (tmp_img + 1) * 127.5 / 255.0
        # torchvision.utils.save_image(tmp_img, 'bg.png')
        # exit()
