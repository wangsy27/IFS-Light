...
class GaussianDiffusion:
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                if rev_mean is not None:
                  ...
                  mask = model_kwargs['faceseg_nohead'].cuda(0)
                  bg_mask = (mask == 0)
                  #print(bg_mask)
                  rgb_tensor = add_mean[i].view(1, 3, 1, 1)
                  # 只在 bg_mask 为 0 的部分加上 rgb_tensor
                  out['sample'] = th.where(bg_mask, out['sample'] + rgb_tensor, out['sample'])
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            new_target = target.cpu().numpy().transpose(0,2,3,1)[0]
            new_output = model_output.detach().cpu().numpy().transpose(0,2,3,1)[0]
            target_emb = get_face_emb(new_target)
            output_emb = get_face_emb(new_output)
            terms["faceemb"] = 0.1 * th.nn.functional.cosine_similarity(target_emb, output_emb, eps=1e-8)
            
            # ima = cv2.cvtColor(new_target, cv2.COLOR_BGR2LAB)
            # imb = cv2.cvtColor(new_output, cv2.COLOR_BGR2LAB)
            # La = ima[:, :, 0]
            # ba = ima[:, :, 2]
            # itA = (La-50)/ba
            # Lb = imb[:, :, 0]
            # bb = imb[:, :, 2]
            # itB = (Lb-50)/bb
            # terms["ita"] = np.abs(itB-itA)

            # import torchvision
            # torchvision.utils.save_image(target, '1.png')
            # torchvision.utils.save_image(model_output, '2.png')

            if "vb" in terms:
                terms["loss"] = terms["mse"] + terms["vb"] + terms["faceemb"]
            else:
                terms["loss"] = terms["mse"] + terms["faceemb"]
        else:
...
import sys
sys.path.insert(0, '/home/wsy/difareli_code/preprocess_scripts/Relighting_preprocessing_tools/Arcface/')
import models

def get_face_emb(target):
    '''
    get face embedding
    '''
    arcface_ckpt_path = '/home/wsy/difareli_code/preprocess_scripts/Relighting_preprocessing_tools/Arcface/pretrained/BEST_checkpoint_r18.tar'
    checkpoint = th.load(arcface_ckpt_path, map_location="cuda:0")
    model = checkpoint['model'].module.to("cuda:0")
    model.eval()
    target = cv2.resize(target, (112, 112))
    ...
    target = data_transforms['normalize'](target)
    target = target.to("cuda:0")
    target = target.reshape(1,3,112,112)
    features = model(target.float())
    return features

# def ITA(imageA, imageB, mask):
    
#     imagA = cv2.resize(cv2.imread(imageA), (256, 256))
#     imagB = cv2.resize(cv2.imread(imageB), (256, 256))
#     ima = cv2.cvtColor(imagA, cv2.COLOR_BGR2LAB)
#     imb = cv2.cvtColor(imagB, cv2.COLOR_BGR2LAB)
#     La = ima[:, :, 0]
#     ba = ima[:, :, 2]
#     ITa = np.arctan((La - 50) / ba) * (180 / math.pi)
#     mean_ITa = np.mean(ITa[mask == 1])

#     Lb = imb[:, :, 0]
#     bb = imb[:, :, 2]
#     ITb = np.arctan((Lb - 50) / bb) * (180 / math.pi)

#     mean_ITb = np.mean(ITb[mask == 1])

#     return np.abs(mean_ITa - mean_ITb)