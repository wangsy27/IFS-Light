...
class RelightNet(nn.Module):
    ...
    def forward(self, img, epoch, intrinsic_matrix, mask, target_lighting, target_ambient_values):
        ...
        for i in range(self.batch_size):
            ...
            light_x = np.ndarray.item(light_xy[0, 0, 0].cpu().detach().numpy())
            light_y = np.ndarray.item(light_xy[1, 0, 0].cpu().detach().numpy())
        ...
        #final_shading = shadow_mask_weights.cuda()*full_shading
...
def main():
    model = RelightNet()
    model.load_state_dict(torch.load('model_lighting_transfer/model_epoch106.pth'))
    model = model.float()
    model = model.cuda()
    model.eval()
    print(model)
    ...
    with torch.no_grad():
        ...
        name_parts = str(reference_image.split('/')[-1].split('.')[0])
