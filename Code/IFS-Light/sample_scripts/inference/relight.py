...
#python relight.py --dataset ffhqvalid --set valid --step 080000 --out_dir ./ffhq/ --cfg_name ifslight_256.yaml --log_dir ours --diffusion_steps 1000 --timestep_respacing 250 --sample_pair_json ./relight.json --sample_pair_mode pair --itp render_face --itp_step 2 --batch_size 1 --gpu_id 0 --lerp --idx 0 10
...
SAMPLE_ROOT = PROJECT_ROOT / "sample_scripts"
FFHQ_SAMPLE_ROOT = PROJECT_ROOT / "preprocess_scripts" / "ffhq"
...
def make_condition(cond, src_idx, dst_idx, src_id, dst_id, n_step=5, itp_func=None):
...
def relight(dat, model_kwargs, itp_func, src_id, dst_id, n_step=3, src_idx=0, dst_idx=1):
    '''
...
if __name__ == '__main__':
    ...
    if args.dataset == 'itw':
        ...
        img_dataset_path = f"/home/user/FairMPdata/aligned_images/"
        deca_dataset_path = f"/home/user/FairMPdata/params/"
        img_ext = '.png'
        cfg.dataset.training_data = f'/MultiPIE/{sub_f}/'
        cfg.dataset.root_path = f'/home/user/FairMPdata'
        cfg.dataset.data_dir = f'/home/user/FairMPdata/aligned_images/'
    elif args.dataset == 'ffhqvalid':
        img_dataset_path = str(FFHQ_SAMPLE_ROOT / 'aligned_images')
        deca_dataset_path = str(FFHQ_SAMPLE_ROOT / 'params')
        img_ext = '.png'
        cfg.dataset.training_data = '/test_images/'
        cfg.dataset.root_path = str(FFHQ_SAMPLE_ROOT)
        cfg.dataset.data_dir = str(FFHQ_SAMPLE_ROOT / 'aligned_images')
    else: raise ValueError


    cfg.dataset.deca_dir = str(FFHQ_SAMPLE_ROOT / "params")
    cfg.dataset.face_segment_dir = str(FFHQ_SAMPLE_ROOT / "face_segment")
    cfg.dataset.deca_rendered_dir = str(FFHQ_SAMPLE_ROOT / "rendered_images")
    cfg.dataset.albedo_rendered_dir = str(FFHQ_SAMPLE_ROOT / "albedo_images")
    ...
    img_path = file_utils._list_image_files_recursively(str(FFHQ_SAMPLE_ROOT / "aligned_images"))
    ...
    for i in range(start, end):
        ...
        if is_render:
            ...
            if clip_ren:
                ...
                #vis_utils.save_images(path=f"{save_res_dir}", fn="ren", frames=out_render)
            else:
