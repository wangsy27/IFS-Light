...
# from test.visualizer.visualizer import get_local
# get_local.activate() # 激活装饰器

# def hook_fn(module, input, output):
#     print("Extracted sim values:", getattr(module, 'attn', None))
...
class ModelFC4(Model):
    ...
    def predict(self, img: Tensor, return_steps: bool = False) -> Union[Tensor, Tuple]:
        ...
        if USE_CONFIDENCE_WEIGHTED_POOLING:
            ...
            skin_pred, light_pred = self._network(img)
            # pred = self._network(img, mimic)
            # print(pred)
            # indices = torch.arange(0, pred.size(0), 3, dtype=torch.long)
            # pred1 = pred[indices+1]
        return skin_pred, light_pred

    def optimize(self, img: Tensor, skin_label: Tensor, light_label: Tensor) -> float:
        self._optimizer.zero_grad()
        skin_pred, light_pred = self.predict(img)
        print(skin_pred, light_pred)
        # indices = torch.arange(0, pred.size(0), 3, dtype=torch.long)
        # pred1 = pred[indices+1]
        # print(pred1)
        loss = self.get_loss(skin_pred, light_pred, skin_label, light_label)
        loss.backward()
