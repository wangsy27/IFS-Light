...
class Model(nn.Module):
    def __init__(self):
        ...
        self.skin_loss_fn = nn.MSELoss()  # 肤色任务 Loss
        self.light_loss_fn = nn.MSELoss()  # 光色任务 Loss
    ...
    def get_loss(self, skin_pred: Tensor, light_pred: Tensor, skin_label: Tensor, light_label: Tensor) -> Tensor:
        skin_loss = self.skin_loss_fn(skin_pred, skin_label)
        #skin_loss = self._criterion(skin_pred, skin_label)
        light_loss = self.light_loss_fn(light_pred, light_label)
        print(skin_loss, light_loss)
        return skin_loss + light_loss
