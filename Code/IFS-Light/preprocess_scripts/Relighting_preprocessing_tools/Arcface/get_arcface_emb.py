...
def get_arcface_emb(img_path, device, arcface_ckpt_path='../../cond_utils/arcface/pretrained/BEST_checkpoint_r18.tar'):
    ...
    # dataset = FFHQlmdb(data='/home/lcx/difareli_code/data/ffhq_256_with_anno')
    loader = DataLoader(
        dataset,
        num_workers=24,
        batch_size=1000,
    ...
    with th.no_grad():
        for i, input_image in tqdm.tqdm(enumerate(loader), desc='Generate Face Embedding'):
            #input_image, index = input_images['img'], input_images['index']
    ...
    #     emb_dict[str(i+1)] = {'faceemb':emb[i]}
