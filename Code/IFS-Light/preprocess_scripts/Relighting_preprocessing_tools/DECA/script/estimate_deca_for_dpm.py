...
#ffhq->datasets1, others->datasets
...
parser.add_argument('--save_images_folder', default='TestSamples/examples/results', type=str,
                    help='path to the output directory, where results(obj, txt files) will be stored.')
parser.add_argument('--save_albedo_folder', default='TestSamples/examples/albedo', type=str,
                    help='path to the albedo directory, where results(obj, txt files) will be stored.')
parser.add_argument('--skin_params', default='TestSamples/examples/params', type=str,
                    help='path to the output directory, where results(obj, txt files) will be stored.')
...
def main():
    ...
    if args.masking_flame:
        f_mask = np.load(f'/home/user/IFS-Light/experimental/masking_flame/FLAME_masks_face-id.pkl', allow_pickle=True, encoding='latin1')
        v_mask = np.load(f'/home/user/IFS-Light/experimental/masking_flame/FLAME_masks.pkl', allow_pickle=True, encoding='latin1')
        mask={
    ...
    albedo_clip_path = f"{args.save_albedo_folder}_wclip/{args.set}"
    albedo_woclip_path = f"{args.save_albedo_folder}_woclip/{args.set}"
    os.makedirs(clip_path, exist_ok=True)
    os.makedirs(woclip_path, exist_ok=True)
    os.makedirs(albedo_clip_path, exist_ok=True)
    os.makedirs(albedo_woclip_path, exist_ok=True)
    ...
    for i in tqdm(data_iters):
        ...
        with th.no_grad():
            ...
            if not args.fast_save_params:
                ...
                skincolor = read_params(path=f"{args.skin_params}/ffhq-valid-skincolor-anno.txt")

                #skins = {1:[244, 237, 229], 2:[241, 231, 220], 3:[245, 234, 211], 4:[231, 219, 190], 5:[211, 190, 155], 6:[155, 127 , 92], 7:[124, 94, 71], 8:[91, 66, 54], 9:[57, 49, 43], 10:[40, 36, 32]}
                skincolor = [skincolor[0]*255.0, skincolor[1]*255.0, skincolor[2]*255.0]
                
                #在这里加skin-color的albedo 
                albedo_image = orig_visdict['albedo_images']
                albedo_image = albedo_image.permute((0, 2, 3, 1))   # BxHxWxC


                for j in range(albedo_image.shape[0]):
                    save_image(tensor=albedo_image[j].permute((2, 0, 1)).cpu(), fp=f"{albedo_clip_path}/{name}.png")
                    image = cv2.imread(f"{albedo_clip_path}/{name}.png")
                    #target_color = skins[skincolor[name+'.png'][0]]
                    target_color = skincolor
                    target_color = target_color[::-1]
                    black_color = [0, 0, 0]
                    albedo = albedo_image[j].cpu().numpy()
                    is_black = np.all(albedo == black_color, axis=-1)
                    albedo[~is_black] = target_color

                    np.save(file=f"{albedo_woclip_path}/{name}.npy", arr=albedo)
                    #save_image(tensor=th.tensor(albedo).permute((2, 0, 1)), fp=f"{albedo_clip_path}/{name}.png")
                    cv2.imwrite(f"{albedo_clip_path}/{name}.png", albedo)

                    # np.save(file=f"{albedo_woclip_path}/{name}.npy", arr=albedo_image[j].cpu().numpy())
                    # save_image(tensor=albedo_image[j].permute((2, 0, 1)).cpu(), fp=f"{albedo_clip_path}/{name}.png")
        ...
        if args.save_params_folder is not None:
            for k, fo in fo_dict.items():
                if k == 'tform':
                    ...
                    fo.write(name + f"{ext} ")
                    #fo.write(name + f" ")
                    fo.write(" ".join([str(x) for x in tform]) + "\n")
                elif k=='albedo':
...
#python estimate_deca_for_dpm.py --useTex False -i /dataset/mst/resize-4/ --useTemplate False --useAvgCam False --useAvgTform False --save_images_folder /dataset/data/ffhq_256_with_anno/albedo_images/deca_masked_face_images --set valid --params_prefix valid --masking_flame
