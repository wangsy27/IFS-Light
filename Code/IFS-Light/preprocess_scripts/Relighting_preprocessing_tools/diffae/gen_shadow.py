...
if __name__ == '__main__':
    ...
    lat = torch.load('/home/wsy/difareli_code/preprocess_scripts/Relighting_preprocessing_tools/diffae/checkpoints/ffhq256_autoenc/latent.pkl', map_location='cpu')
    ...
    with open(f'{args.out_dir}/ffhq-{args.set_}-shadow-anno.txt', 'w') as f:
        for i in range(ndata.shape[0]):
            #如果是ffhq注释掉第一句
            # m = im_name[i].split('/')[-1]
            # f.write(f'{i+1}.png {proj_data[i].item()}')
            # f.write('\n')
            m = im_name[i].split('/')[-1]
            f.write(f'{m} {proj_data[i].item()}')
            f.write('\n')
#python gen_shadow.py --image_dir /home/lcx/difareli_code/preprocess_scripts/Relighting_preprocessing_tools/test_images/aligned_images/skin_color/ --out_dir ./shadow_out